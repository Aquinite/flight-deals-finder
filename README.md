...
3. **Finds the cheapest flight per destination** across all dates checked. Flight price is based on a 1-week trip but is configurable as well inside the code. 
...
- Adding user-friendly way to change the set of dates for checking flight availability.
- Letting other users sign up through a Google Form and letting Sheety read the responses in order to send email alerts to signed-up users using SMTPlib. 
...
2. Create a `.env` file (or set environment variables) with:
   ...
   ```
3. Set up a Google Sheet with columns for destination IATA code and lowest recorded price, connected via Sheety.
4. Run the script:
   ```
   python main.py
   ```
